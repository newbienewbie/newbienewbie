...
出于某种原因，有一天我希望在函数原有功能之上追加一点工作量——在函数执行之前和之后实现一些和 foo 函数完全可以分离的工作，我们当然不想直接改动已经写好的 foo ,所以自然而然想去定义一个新函数，在这个函数内部调用原函数：
...
```
这种做法，对于一些小程序当然没有问题，但是对于稍微大的项目或者涉及到接口的时候情况就明显不同了：假如用户（可以是其他程序员，也可以是自己）使用了旧函数 foo() ，就得把之前所有的 foo() 全部改成 foo_appended(foo)，这是个很糟糕的事情。
更关键的，这样一点都不 [AOP](http://www.itminus.com/tags/AOP/)！

### 装饰器实现

理论基础是`Python一切皆对象，包括函数`。所以可以对函数进行普通对象一样的操作：接收作为参数、传递为返回值、赋值（引用新对象）。
在不改动原 foo() 内部实现代码的情况下，想在 foo() 原有工作前后追加工作量(甚至完全替换掉 foo 的工作)，这需要一个偷梁换柱的方法来实现：
...
Python装饰器能扩展原函数，核心在于动态生成了一个包装函数，该包装函数负责扩展原函数(甚至完全替换掉原函数的所有工作)。最后把该包装函数当原函数一样使用。
...
可以把第2步、第3步合并为一步，但是根本要旨是要返回一个函数对象（对原函数进行包装，不妨称之为包装函数）。
...
related:Python内置了类属性@property、类静态方法@staticmethod、类方法classmethod@装饰器。这三个是和面向对象相关的。

## 装饰器的应用

装饰器的应用非常多，在`Django`中随处可见诸如`@require_login`检查登陆之类的功能。
